...
이번에는 기능 목록 26번인 "오른쪽 벽 반사"만 만들고 싶어.
...
목표:
발사된 버블이 오른쪽 벽(RightWall)에 닿으면 왼쪽으로 반사되게 하고 싶어.
벽에 닿은 뒤에도 속도가 유지되어야 해.

조건:
- Unity 6 2D 프로젝트 기준으로 설명해줘.
- 이번에는 기능 26번 "오른쪽 벽 반사"만 만들어줘.
- BubbleLauncherController.cs에 이미 PhysicsMaterial2D로 벽 반사가 구현되어 있는지 확인해줘.
- 이미 구현되어 있으면 확인만 해줘.
- 구현되어 있지 않으면 PhysicsMaterial2D를 사용해서 벽 반사를 구현해줘.
- 오른쪽 벽(RightWall)에 닿으면 왼쪽으로 반사돼야 해.
- 벽에 닿은 뒤에도 속도가 유지되어야 해.
...
코드 작성 조건:
- BubbleLauncherController.cs의 벽 반사 관련 설정만 확인/수정해줘.
...
3. 기능 26번이 성공하면 다음에는 기능 27번 프롬프트를 만들면 됩니다.
