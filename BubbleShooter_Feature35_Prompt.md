...
이번에는 기능 목록 35번인 "버블 제거 시 점수 증가"만 만들고 싶어.
...
- 이번에는 기능 35번 "버블 제거 시 점수 증가"만 만들어줘.
...
- BubbleGridManager의 MatchedBubblesRemoved 이벤트를 Awake나 Start에서 구독해줘.
- event 구독은 "+=" 연산자로 해줘.
...
- AddBubbleScore 함수가 이미 있으니 그걸 호출만 해줘.
...
중요한 테스트 상황:
- 같은 색 3개를 제거하면 점수가 올라가는지 확인.
- 점수가 몇 점 올라가는지 확인. (예: 버블 1개당 10점이면 3개 제거 시 30점)
- 같은 색 3개 + 옆 버블 1개 = 4개 제거 시 점수가 40점인지 확인.
