...
이번에는 기능 목록 28번인 "천장에 붙기"만 만들고 싶어.
...
현재 상태:
- 기능 1번~27번은 완료했어. (22번 모바일 터치는 패스)
...
목표:
발사된 버블이 천장(Ceiling)에 닿으면 그 자리에 멈추고 고정되어야 해.
천장에 붙은 버블은 떨어지지 않아야 해.

조건:
- Unity 6 2D 프로젝트 기준으로 설명해줘.
- 이번에는 기능 28번 "천장에 붙기"만 만들어줘.
- BubbleLauncherController.cs의 BubbleCollisionHandler에 이미 천장 충돌 처리가 있는지 확인해줘.
- 이미 구현되어 있으면 확인만 해줘.
- 구현되어 있지 않으면 천장 충돌 처리를 구현해줘.
- 천장에 닿으면 Rigidbody2D를 Static으로 바꿔서 고정시켜줘.
- 천장에 붙은 버블은 떨어지지 않아야 해.
...
코드 작성 조건:
- BubbleLauncherController.cs의 천장 충돌 관련 설정만 확인/수정해줘.
...
3. 기능 28번이 성공하면 다음에는 기능 29번 프롬프트를 만들면 됩니다.
