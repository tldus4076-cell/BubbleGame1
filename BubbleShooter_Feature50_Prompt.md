# 기능 50 프롬프트: Stage 1 난이도 - 제한 샷 수를 넉넉하게 주기
...
이번에는 기능 목록 50번인 "Stage 1 난이도 - 제한 샷 수를 넉넉하게 주기"만 만들고 싶어.
...
현재 상태:
- 기능 1번~49번은 완료했어. (22번 모바일 터치는 패스)
- 기능 49번 "Stage 1 난이도 - 시작 배치를 단순하게 만들기" 테스트 완료했어.
- StageDataController.cs가 이미 있어서 Inspector에서 maxShotCount를 저장할 수 있어.
- 기능 45에서 maxShotCount 변수와 GetMaxShotCount() 함수를 만들었어.
- 아직 Stage 1에서 실제로 넉넉한 제한 샷 수가 적용되어 있지 않어.
- 기존 배경, 타이머, 게이지, 점수, 슈터, 조준선, 현재 버블, 다음 버블, 발사, 격자, 같은 색 찾기, 버블 제거, 점수 증가, 제거 효과, 천장 연결 찾기, 떠 있는 버블 찾기, 떠 있는 버블 떨어뜨리기, 떨어진 버블 점수, 스테이지 이름, 배경 이미지, 색 종류, 배치, 제한 샷 수, 제한 시간, 장애물, 3색, 단순 배치 기능은 절대 망가뜨리지 말아줘.
- EventSystem은 삭제된 상태야. 이번 기능에는 필요 없으면 만들지 마.

목표:
Stage 1의 제한 샷 수를 넉넉하게 주고 싶어.
초보자가 여러 번 실수해도 클리어할 수 있도록 25발로 설정하고 싶어.

조건:
- Unity 6 2D 프로젝트 기준으로 설명해줘.
- 이번에는 기능 50번 "Stage 1 난이도 - 제한 샷 수를 넉넉하게 주기"만 만들어줘.
- StageDataController.cs를 수정하지 마. (이미 maxShotCount가 있음)
- 기존 코드는 건드리지 마.
- Stage 1 씬의 Inspector 설정만 확인하고 안내해줘.
- StageDataController의 Inspector에서:
  - Max Shot Count: 25 설정
- 추천 제한 샷 수:
  - Stage 1: 25발 (넉넉함)
  - Stage 2: 22발 (보통)
  - Stage 3: 18발 (어려움)
- 초보자가 여러 번 실수해도 클리어할 수 있도록 25발로 설정.
- 나중에 Stage 2, Stage 3을 만들면 더 적은 샷 수를 넣으면 됨.
...
중요한 테스트 상황:
- Inspector에서 Max Shot Count가 25로 설정되어 있는지 확인.
- Play하면 Console에 "[기능 45] Stage 1 제한 샷 수: 25발" 로그가 출력되는지 확인.
- 기존 발사/정렬/같은 색 찾기/버블 제거/점수 증가/제거 효과/천장 연결 찾기/떠 있는 버블 찾기/떠 있는 버블 떨어뜨리기/떨어진 버블 점수/스테이지 이름/배경 이미지/색 종류/배치/제한 샷 수/제한 시간/장애물/3색/단순 배치 기능이 정상 작동하는지 확인.
...
- 왜 Stage 1에서 넉넉한 제한 샷 수가 필요한지 알려줘.
- 초보자가 실수해도 클리어할 수 있는 이유를 알려줘.
...
3. 기능 50번이 성공하면 다음에는 기능 51번 프롬프트를 만들면 됩니다.
