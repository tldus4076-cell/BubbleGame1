...
이번에는 기능 목록 2번인 "Stage 2 배경 이미지 넣기"만 만들고 싶어.
...
- StageBackgroundController.cs가 이미 있을 수 있어.
- 기존 Stage 1 배경 기능은 절대 망가뜨리지 말아줘.

목표:
내가 준비한 Stage 2 배경 이미지를 추가하고 싶어.
아직 스테이지 전환 기능은 만들지 말고, Stage 2 배경 Sprite를 나중에 쓸 수 있게 준비하고 싶어.

조건:
- Unity 6 2D 프로젝트 기준으로 설명해줘.
- 배경 이미지는 Sprite로 사용할 예정이야.
- 이번에는 Stage 2 배경 이미지 1장만 추가해줘.
- Stage 1 배경 기능은 그대로 유지해줘.
- Stage 2 배경은 나중에 Stage 2를 시작할 때 사용할 수 있게 변수로 준비해줘.
- 아직 Stage 1에서 Stage 2로 바뀌는 기능은 만들지 마. 그건 기능 4번에서 할 예정이야.
...
코드 작성 조건:
- 기존 StageBackgroundController.cs를 수정하는 방식이면 기존 Stage 1 기능을 유지해줘.
- 새 스크립트가 더 안전하다면 왜 새 스크립트가 필요한지 설명해줘.
- 가능하면 기존 StageBackgroundController.cs에 Stage 2 배경 Sprite 변수만 추가하는 쉬운 방식으로 해줘.
- [Header("한글 설명")]을 사용해줘.
...
- 기존 StageBackgroundController.cs에 Stage 2 배경 Sprite 변수를 추가해줘.
- 변수 이름 예시는 stage2BackgroundSprite로 해줘.
- Inspector에서 Stage 2 배경 Sprite를 연결할 수 있게 해줘.
- 아직 기본으로 보이는 배경은 Stage 1 그대로 두는 것이 좋아.
- Stage 2 배경이 제대로 연결되었는지 확인하는 함수나 방법을 알려줘.
...
주의:
- 이번에는 기능 2번만 만들어줘.
- Stage 2로 실제 전환하는 기능은 만들지 마. 그건 기능 4번에서 만들 거야.
...
4. 기능 2번이 성공하면 다음에는 기능 3번 프롬프트를 만들면 됩니다.
