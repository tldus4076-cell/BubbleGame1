...
이번에는 기능 목록 46번인 "Stage별 제한 시간 저장"만 만들고 싶어.
...
현재 상태:
- 기능 1번~45번은 완료했어. (22번 모바일 터치는 패스)
- 기능 45번 "Stage별 제한 샷 수 저장" 테스트 완료했어.
- StageDataController.cs가 이미 있어서 Inspector에서 스테이지 이름, 번호, 배경 이미지, StageBackgroundController, 버블 색 Sprite[], colorCount, startRows, startCols, startBubblePattern, maxShotCount를 저장할 수 있어.
- 아직 StageDataController에서 스테이지별 제한 시간을 저장하는 기능이 없어.
- 기존 배경, 타이머, 게이지, 점수, 슈터, 조준선, 현재 버블, 다음 버블, 발사, 격자, 같은 색 찾기, 버블 제거, 점수 증가, 제거 효과, 천장 연결 찾기, 떠 있는 버블 찾기, 떠 있는 버블 떨어뜨리기, 떨어진 버블 점수, 스테이지 이름, 배경 이미지, 색 종류, 배치, 제한 샷 수 기능은 절대 망가뜨리지 말아줘.
- EventSystem은 삭제된 상태야. 이번 기능에는 필요 없으면 만들지 마.

목표:
스테이지별로 제한 시간을 저장하고 싶어.
Stage 1은 넉넉한 90초, Stage 2는 75초, Stage 3은 60초처럼 스테이지마다 다르게 설정하고 싶어.

조건:
- Unity 6 2D 프로젝트 기준으로 설명해줘.
- 이번에는 기능 46번 "Stage별 제한 시간 저장"만 만들어줘.
- StageDataController.cs를 수정해줘.
- 다른 스크립트는 수정하지 마.
- StageDataController.cs에 제한 시간 변수를 추가해줘:
  - [SerializeField] private float timeLimit = 90f; (이 스테이지의 제한 시간, 초 단위)
- Inspector에서 timeLimit를 입력하면 해당 스테이지의 제한 시간이 됨.
- Stage 1은 90초, Stage 2는 75초, Stage 3은 60초를 추천합니다.
- 나중에 스테이지가 바뀌면 Inspector 값만 바꾸면 되게 해줘.
- 다른 스크립트에서 timeLimit를 가져갈 수 있는 public 함수를 만들어줘:
  - GetTimeLimit() 함수: 현재 timeLimit를 return 해줘.
- ShowStageInfo()에서 제한 시간 로그도 출력해줘:
  - [기능 46] Stage {stageNumber} 제한 시간: {timeLimit}초
...
중요한 테스트 상황:
- Inspector에서 timeLimit를 90으로 설정.
- Play하면 Console에 "[기능 46] Stage 1 제한 시간: 90초" 로그가 출력되는지 확인.
- Inspector에서 timeLimit를 75로 바꾸면 로그도 75로 바뀌는지 확인.
- GetTimeLimit() 함수가 올바른 숫자를 반환하는지 확인.
- 기존 발사/정렬/같은 색 찾기/버블 제거/점수 증가/제거 효과/천장 연결 찾기/떠 있는 버블 찾기/떠 있는 버블 떨어뜨리기/떨어진 버블 점수/스테이지 이름/배경 이미지/색 종류/배치/제한 샷 수 기능이 정상 작동하는지 확인.
...
- 왜 스테이지별로 제한 시간을 다르게 해야 하는지 알려줘.
...
3. 기능 46번이 성공하면 다음에는 기능 47번 프롬프트를 만들면 됩니다.
