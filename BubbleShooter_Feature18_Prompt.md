...
이번에는 기능 목록 18번인 "현재 버블 표시"만 만들고 싶어.
...
목표:
지금 발사할 버블을 슈터 위에 보여주고 싶어.
발사 전 현재 버블 색이 보이면 성공이야.

조건:
- Unity 6 2D 프로젝트 기준으로 설명해줘.
- 이번에는 기능 18번 "현재 버블 표시"만 만들어줘.
- 버블 발사는 아직 만들지 마. 그건 기능 21번에서 만들 거야.
- 현재 버블은 슈터 위에 보여야 해.
- 현재 버블은 배경보다 앞에 보여야 해.
- 현재 버블은 마우스/키보드 조준 방향과 함께 회전하지 않아도 돼. (고정된 위치에 표시)
...
현재 버블 표시 방식:
- BubbleCurrentController.cs 같은 새 스크립트를 만들어줘.
- BubbleCurrentController.cs는 ShooterRoot에 붙이는 방식으로 해줘.
- 현재 버블 Sprite를 Inspector에서 연결하는 방식으로 해줘.
- 현재 버블 색을 랜덤으로 정할지 Inspector에서 선택하게 해줘.
- bubbleColors 배열로 색 종류를 관리하게 해줘.
- currentBubbleSprite 변수로 현재 버블 Sprite를 관리하게 해줘.
- showCurrentBubble bool로 표시 여부를 관리하게 해줘.
- sortingOrder로 배경보다 앞에 보이게 해줘.

코드 작성 조건:
- BubbleCurrentController.cs를 새로 만들어줘.
- 기존 ShooterAimController.cs, ShooterAimLineController.cs는 가능하면 건드리지 마.
- Awake/Start에서 초기화해줘.
- Update에서 필요시 갱신해줘.
- [Header("한글 설명")]을 사용해줘.
...
- 어떤 GameObject에 어떤 스립트가 붙어 있어야 하는지 알려줘.
...
3. 기능 18번이 성공하면 다음에는 기능 19번 프롬프트를 만들면 됩니다.