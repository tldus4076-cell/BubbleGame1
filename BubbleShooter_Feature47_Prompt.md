...
이번에는 기능 목록 47번인 "Stage별 장애물 유무 저장"만 만들고 싶어.
...
현재 상태:
- 기능 1번~46번은 완료했어. (22번 모바일 터치는 패스)
- 기능 46번 "Stage별 제한 시간 저장" 테스트 완료했어.
- StageDataController.cs가 이미 있어서 Inspector에서 스테이지 이름, 번호, 배경 이미지, StageBackgroundController, 버블 색 Sprite[], colorCount, startRows, startCols, startBubblePattern, maxShotCount, timeLimit를 저장할 수 있어.
- 아직 StageDataController에서 스테이지별 장애물 유무를 저장하는 기능이 없어.
- 기존 배경, 타이머, 게이지, 점수, 슈터, 조준선, 현재 버블, 다음 버블, 발사, 격자, 같은 색 찾기, 버블 제거, 점수 증가, 제거 효과, 천장 연결 찾기, 떠 있는 버블 찾기, 떠 있는 버블 떨어뜨리기, 떨어진 버블 점수, 스테이지 이름, 배경 이미지, 색 종류, 배치, 제한 샷 수, 제한 시간 기능은 절대 망가뜨리지 말아줘.
- EventSystem은 삭제된 상태야. 이번 기능에는 필요 없으면 만들지 마.

목표:
스테이지별로 장애물이 있는지 없는지를 저장하고 싶어.
Stage 1은 장애물 없음, Stage 2는 장애물 없음 또는 적음, Stage 3은 장애물 있음으로 설정하고 싶어.

조건:
- Unity 6 2D 프로젝트 기준으로 설명해줘.
- 이번에는 기능 47번 "Stage별 장애물 유무 저장"만 만들어줘.
- StageDataController.cs를 수정해줘.
- 다른 스크립트는 수정하지 마.
- StageDataController.cs에 장애물 유무 변수를 추가해줘:
  - [SerializeField] private bool hasObstacles = false; (이 스테이지에 장애물이 있는지 여부)
- Inspector에서 hasObstacles를 체크하면 해당 스테이지에 장애물이 있는 것으로 설정됨.
- Stage 1은 체크 해제(false), Stage 3은 체크(true)를 추천합니다.
- 나중에 스테이지가 바뀌면 Inspector 값만 바꾸면 되게 해줘.
- 다른 스크립트에서 hasObstacles를 가져갈 수 있는 public 함수를 만들어줘:
  - HasObstacles() 함수: 현재 hasObstacles를 return 해줘.
- ShowStageInfo()에서 장애물 유무 로그도 출력해줘:
  - [기능 47] Stage {stageNumber} 장애물: 있음 또는 없음
...
중요한 테스트 상황:
- Inspector에서 hasObstacles를 체크 해제하면 "없음" 로그가 출력되는지 확인.
- Inspector에서 hasObstacles를 체크하면 "있음" 로그가 출력되는지 확인.
- HasObstacles() 함수가 올바른 값을 반환하는지 확인.
- 기존 발사/정렬/같은 색 찾기/버블 제거/점수 증가/제거 효과/천장 연결 찾기/떠 있는 버블 찾기/떠 있는 버블 떨어뜨리기/떨어진 버블 점수/스테이지 이름/배경 이미지/색 종류/배치/제한 샷 수/제한 시간 기능이 정상 작동하는지 확인.
...
- 왜 스테이지별로 장애물 유무를 다르게 해야 하는지 알려줘.
- Inspector에서 체크박스를 사용하는 것이 왜 편한지 쉽게 설명해줘.
- bool 변수가 무엇인지 쉽게 설명해줘. (true/false, 있음/없음)
- 함수별 설명을 해줘.
...
3. 기능 47번이 성공하면 다음에는 기능 48번 프롬프트를 만들면 됩니다.
